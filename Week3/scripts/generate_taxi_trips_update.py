...
def _sample_and_shift(raw: DataFrame, fraction: float, shift_seconds: int) -> DataFrame:
    sampled = raw.sample(withReplacement=False, fraction=fraction, seed=RANDOM_SEED)

    jittered = (
        sampled
        .withColumn("_jitter_s", (rand(seed=RANDOM_SEED) * shift_seconds).cast("long"))
        .withColumn(
            "tpep_pickup_datetime",
            expr("tpep_pickup_datetime + make_interval(0,0,0,0,0,0,_jitter_s)"),
        )
        .withColumn(
            "tpep_dropoff_datetime",
            expr("tpep_dropoff_datetime + make_interval(0,0,0,0,0,0,_jitter_s)"),
        )
        .drop("_jitter_s")
    )
    return jittered
...
def generate() -> dict:
    ...
    shift_expr = f"make_interval(0,0,0,0,0,0,{delta_seconds})"
    new_trips = (
        new_sample
